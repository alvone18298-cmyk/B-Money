...
async function sendOTPEmail(toEmail, otp) {
  try {
    await mailer.sendMail({
      from: `"B-Money 💰" <${GMAIL_USER}>`,
      to: toEmail,
      subject: `Your B-Money OTP: ${otp}`,
      html: `
      <div style="font-family:Arial,sans-serif;max-width:420px;margin:0 auto;background:#f0f2f8;padding:30px;border-radius:16px">
        <h2 style="color:#1a1aff;text-align:center;margin-bottom:4px">B-<span style="color:#ffd700">Money</span></h2>
        ...
        <p style="color:#aaa;font-size:11px;text-align:center">Do not share this OTP with anyone. B-Money will never ask for your OTP.</p>
      </div>`
    });
    return { success: true };
  } catch (e) {
...
app.post('/api/deposit/request', auth, async (req, res) => {
  try {
    ...
    const confirmUrl = `${process.env.APP_URL || 'https://bmoney-app.vercel.app'}/api/admin/confirm-deposit?id=${dep._id}&key=${process.env.ADMIN_SECRET || 'bm_admin_2024'}`;
    const rejectUrl  = `${process.env.APP_URL || 'https://bmoney-app.vercel.app'}/api/admin/reject-deposit?id=${dep._id}&key=${process.env.ADMIN_SECRET || 'bm_admin_2024'}`;
...
// ===================== AUTO-WITHDRAWAL (Toggle trigger — MAX balance, no cooldown) =====================
...
app.get('*', (req, res) => {
  ...
  res.send('B-Money API ✅');
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`🚀 B-Money running on port ${PORT}`));
